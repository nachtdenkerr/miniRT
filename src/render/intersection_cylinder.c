...
/*   Updated: 2025/08/06 12:38:51 by thudinh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*solve_t_values(t_cylinder *cyl, t_ray *ray)
{
	...
	if (discriminant < 0)
		return (NULL);
	return ((double []){(-h - sqrt(discriminant)) / a,
		(-h + sqrt(discriminant)) / a});
}

bool	hit_cylinder_body(t_cylinder *cyl, t_ray *ray,
			...
			double *closest_t)
{
	...
	if (fabs(denominator) < EPSILON)
		return (false);
	t = vec_dot(vec_sub(cyl->center, ray->origin), axis) / denominator;
	if (t < 0 || t > *closest_t)
		return (false);
...
bool	hit_cylinder(t_cylinder *cyl, t_ray *ray, t_hit_record *rec)
{
	...
	closest_t = INFINITY;
	hit = false;
	if (hit_cylinder_body(cyl, ray, &tmp_rec, &closest_t))
	{
		*rec = tmp_rec;
		hit = true;
	}
